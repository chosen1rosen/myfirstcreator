...
async function getActiveVariant(domainId = 1) {
  const [modeRaw, sequenceRaw, activeIdRaw, clickCountRaw, startedAtRaw, clickThreshRaw, timeHoursRaw] = await Promise.all([
    getSetting(domainRotKey(domainId, 'mode')),           // 'manual' | 'click' | 'time'
    getSetting(domainRotKey(domainId, 'sequence')),       // JSON array of variant IDs in order
    getSetting(domainRotKey(domainId, 'active_id')),      // current active variant ID
    getSetting(domainRotKey(domainId, 'click_count')),    // visits to current variant this window
    getSetting(domainRotKey(domainId, 'started_at')),     // ISO timestamp when current window started
    getSetting(domainRotKey(domainId, 'click_threshold')),// visits before rotating (click mode)
    getSetting(domainRotKey(domainId, 'time_hours')),     // hours per variant (time mode)
  ]);
...
function renderLandingPage(variant, testimonials, isPreview = false, vslData = null) {
  ...
  const isYouTube = vslSrc && !vslIsFile && /youtube\.com\/embed\//.test(vslSrc);
  const ytVideoId = isYouTube ? (vslSrc.match(/youtube\.com\/embed\/([^?&]+)/) || [])[1] : null;
  const vslUID = 'vsl_' + Math.random().toString(36).slice(2, 8);

  let vslHTML = '';
  if (vslSrc) {
    if (vslIsFile) {
      vslHTML = `<section class="vsl-section"><div class="container"><div class="vsl-wrap" id="wrap-${vslUID}"><video id="${vslUID}" src="${vslSrc}" controls preload="metadata" playsinline></video></div></div></section>
<script>(function(){
  var v=document.getElementById('${vslUID}');
  if(!v)return;
  ...
  setTimeout(function(){
    ...
    v.play().then(function(){
      ...
      setTimeout(function(){
        if(v.muted){
          var wrap=document.getElementById('wrap-${vslUID}');
      ...
      vslHTML = `<section class="vsl-section"><div class="container"><div class="vsl-wrap" id="wrap-${vslUID}"><div id="${vslUID}"></div></div></div></section>
<script>(function(){
  function initPlayer(){
    new YT.Player('${vslUID}',{
      videoId:'${ytVideoId}',
  ...
  if(window.YT&&window.YT.Player){initPlayer();}
    ...
    window.onYouTubeIframeAPIReady=function(){if(prev)prev();initPlayer();};
      ...
      const embedSrc = vslSrc + (vslSrc.includes('?') ? '&' : '?') + 'autoplay=1';
      vslHTML = `<section class="vsl-section"><div class="container"><div class="vsl-wrap"><iframe src="${embedSrc}" frameborder="0" allowfullscreen allow="autoplay; encrypted-media"></iframe></div></div></section>`;
    }
  }

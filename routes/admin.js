...
router.get('/dashboard', requireAuth, async (req, res) => {
  ...
  let signupQ  = supabase.from('signups').select('*', { count: 'exact', head: true }).eq('domain_id', domainId);
  let visitorQ = supabase.from('visitors').select('*', { count: 'exact', head: true });
  let linkQ    = supabase.from('tracking_links').select('*', { count: 'exact', head: true }).eq('domain_id', domainId);
  ...
  const { count: todaySignups } = await supabase.from('signups')
    ...
    .gte('signed_up_at', today);

  const { data: recentSignups } = await supabase.from('signups')
    .select('*').eq('domain_id', domainId)
    .order('signed_up_at', { ascending: false }).limit(10);

  // Tracking links scoped to domain
...
router.get('/signups', requireAuth, async (req, res) => {
  ...
  let query = supabase.from('signups').select('*', { count: 'exact' })
    .eq('domain_id', domainId)
    .order('signed_up_at', { ascending: false }).range(from, from + limit - 1);
...
router.get('/signups/export', requireAuth, async (req, res) => {
  const { data: signups } = await supabase.from('signups').select('*')
    .eq('domain_id', req.currentDomainId)
...
router.post('/tracking', requireAuth, async (req, res) => {
  ...
  const { data: existing } = await supabase.from('tracking_links').select('id')
    .eq('slug', clean).eq('domain_id', domainId).single();
  ...
  await supabase.from('tracking_links').insert({ slug: clean, name: name||null, destination: dest, link_mode: 'global', domain_id: domainId });
